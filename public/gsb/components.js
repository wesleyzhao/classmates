 ...
 * `copyLabel` names the copy button in both cases; `copyClassName` styles it when it sits beside the share button
 * (alone, without a share sheet, it takes `className`, so the one button is the prominent one).
 * @param {{ url: string, title: string, text: string, className?: string, label?: string, copyLabel?: string, copyClassName?: string }} props
 */
export function ShareLink({ url, title, text, className = "btn btn-secondary", label = "Share invite link", copyLabel = null, copyClassName = null }) {
  ...
  return html`<span class="share-link">
    ...
    <button type="button" class=${canShare && copyClassName ? copyClassName : className} onClick=${copy}>${copied ? "Link copied" : copyLabel ?? (canShare ? "Copy link" : "Copy invite link")}</button>
