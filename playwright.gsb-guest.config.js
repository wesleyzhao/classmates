...
export default defineConfig({
  ...
  use: { baseURL: "http://localhost:3139", viewport: { width: 390, height: 844 },
    trace: "retain-on-failure", screenshot: "only-on-failure" },
  projects: [{ name: "chromium", use: { browserName: "chromium" } },
    { name: "webkit", use: { browserName: "webkit" } }],
  webServer: {
    command: `NODE_ENV=test GSB_TEST_SCHEMA=gsb_test_guest node --env-file=.env.local scripts/gsb/test-setup.js && NODE_ENV=test GSB_TEST_SCHEMA=gsb_test_guest GSB_GUEST_PREVIEW=true GSB_GUEST_PERSON_IDS=${ids} GSB_TEST_EMAIL_OUTBOX=/tmp/gsb-guest-mail.jsonl APP_PROFILE=gsb APP_ORIGIN=http://localhost:3139 node --env-file=.env.local scripts/gsb/guest-test-server.js`,
