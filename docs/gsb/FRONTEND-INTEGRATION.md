...
`GuestRound` owns its media through `prepareRoundMedia`. Its first screen is `GuestCountdown`, which starts immediately while the round request and photo decoding run. The first question requires both the countdown and photo preparation to finish; a slow connection holds the final beat. Returning from a hidden tab restarts the countdown. Keep this automatic entry free of an introduction or start button. The app fetches the guest module as soon as the session confirms eligibility. Disabled deployments and signed-in users do not fetch the guest round module or guest content. Answers update synchronously, check the displayed question ID and never wait for an animation or HTTP. `sprint-pieces.js` supplies the same portrait and choice artwork to both guest and signed-in Speed. Keep guest input and result prompts inside the full-screen cabinet: shell-level notices sit behind it. The shared `Login` supports a compact inline email form and displays its own errors.
...
### Two-choice default (September 21, 2026)

Keep the automatic guest 3-2-1 followed by ten faces with **two** named drop targets. `two-door-pieces.js` now shares the artwork and gesture mapping across Guest, solo Speed and Duel. Signed-in quick landing uses the same two-choice interaction, with four-choice face matching retained in Match. Name-to-face and mixed still have four choices. Records and cached/pending runs carry their difficulty; do not strip `choices` from prepare requests/responses or records queries. The existing 20-to-10 buffer still reuses both halves.

Email preview and custom-sender copy are in `server/gsb/login-email.js`. Descope's current live System template cannot use this copy without a custom connector. See OPERATIONS.md for verified quotas and the distinction between app throttles and provider allowance.
